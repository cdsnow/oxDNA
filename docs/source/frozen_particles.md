...
### Softening the frozen boundary to prepare configurations

A movable chain built independently of its frozen surroundings can end up threaded through them; single-particle moves cannot undo that (every path out crosses a wall of excluded volume). `frozen_exc_volume_scale = s` multiplies the repulsive excluded-volume terms between a frozen and a movable particle by `s` (oxRNA nonbonded excluded volume and RNANM protein-RNA excluded volume; hydrogen bonding, stacking and electrostatics with the frozen particles are untouched), so that with a small `s` the chain passes through the boundary while the gradient still pushes it outwards. Ramping `s` back to 1 in a few short stages leaves a configuration that satisfies the real model. With `s != 1` the simulation does not sample the model's Boltzmann distribution, and a warning is logged; production runs must use `s = 1` (the default).
