...
* `[frozen_particles_file = <string>]`: path to a file listing the particles that should be frozen, *i.e.* whose position and orientation are never changed by the simulation. Each line contains whitespace- or comma-separated particle indices or inclusive index ranges (*e.g.* `0-9 15 20-24`); `#` starts a comment. Frozen particles still interact with all the other particles (they act as a rigid boundary). Supported by the `MD`, `MC`, `MC2`, `VMMC` and `PT_VMMC` CPU backends only, and requires `fix_diffusion = false`. Cluster moves (`VMMC`, `Pivot`) that would move a frozen particle are rejected, and volume moves are not allowed.
* `[frozen_skip_bonded_pairs = <bool>]`: if true, bonded interactions between two frozen particles are never evaluated (in full-system energies, MD forces and the topology sanity checks). Their energy is a constant that cannot affect the sampling of the movable particles, and skipping them makes it possible to freeze a reference structure whose backbone geometry is not exactly compatible with the model (a FENE bond outside its range would otherwise contribute an energy of the order of {math}`10^{12}`). Nonbonded interactions between frozen particles are still computed. Defaults to `false`; requires frozen particles; not supported by the `VMMC` and `PT_VMMC` backends. Observables that evaluate bonded terms pair by pair (for instance `pair_energy`) still report the raw value for frozen pairs.
* `[frozen_skip_nonbonded_pairs = <bool>]`: same as `frozen_skip_bonded_pairs` for the nonbonded interactions between two frozen particles (also constants). Useful when the frozen structure contains overlaps that the model's excluded volume would penalise, and to speed up full-system energy computations of mostly-frozen systems. Defaults to `false`.
* `[frozen_exc_volume_scale = <float>]`: factor (default 1) applied to the repulsive excluded-volume terms between a frozen and a movable particle (oxRNA nonbonded excluded volume, RNANM protein-RNA excluded volume). Values below 1 soften the frozen boundary so that a movable chain threaded through it can escape; this changes the model and is meant only for preparing initial configurations, typically ramping 0.02 -> 0.1 -> 0.3 -> 1 before any production run. A warning is logged whenever it differs from 1. RNA/RNANM interactions only.
* `[frozen_strands = <string>]`: comma-separated list of strand ids whose particles are all frozen. Strand ids are the internal, 0-based ones (the first strand of a legacy topology file, numbered 1 there, has id 0), as reported by the `strand_id` attribute of the particles in oxpy. Can be combined with `frozen_particles_file`.
